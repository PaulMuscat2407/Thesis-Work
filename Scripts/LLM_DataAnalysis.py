...
data1 = pd.to_numeric(df['character BERT F1 score medium'].iloc[start_row:end_row + 1])
data2 = pd.to_numeric(df['character BERT F1 score long'].iloc[start_row:end_row + 1])
...
# There is sufficient statistical evidence at tee alpha = 0.05 level of significance to reject H0 in favour Ha.
